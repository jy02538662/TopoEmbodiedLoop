...
def apply_action(state: RobotState, scene: Scene, action: str, release_dir: int, push_scale: float, rng: random.Random) -> None:
    previous = state.contact_state
    if action == "insert":
        state.progress += push_scale * (0.045 * (1.0 - 0.55 * state.tightness) - 0.004 * scene.difficulty)
        state.lateral_error -= push_scale * 0.031 * (1.0 - 0.45 * state.tightness)
        state.force += push_scale * (4.2 + 6.2 * scene.difficulty + 8.2 * state.tightness)
        ...
        state.force += -20.0 * max(match, 0.0) + 9.0 * max(-match, 0.0)
        state.tightness += -0.25 * max(match, 0.0) + 0.10 * max(-match, 0.0)
        state.lateral_error += 0.022 * max(match, 0.0) + 0.02 * max(-match, 0.0)
        state.contact_state = "release" if match > 0 else "jam"
    elif action == "recapture":
        state.recaptured_once = True
        state.lateral_error -= 0.092 * (1.0 - 0.34 * state.tightness)
        state.progress += 0.047 * (1.0 - 0.38 * state.tightness)
        state.force += 0.8 * scene.difficulty - 6.0 * (1.0 - state.tightness)
        state.tightness -= 0.085
